...
class Pack:
    def __init__(self, identifier, version):
        ...
        self.connection.executescript('''
          ...
          CREATE TABLE entries(word TEXT PRIMARY KEY,rank INTEGER NOT NULL,payload BLOB NOT NULL,payload_size INTEGER NOT NULL) WITHOUT ROWID;
    ...
    def insert(self, word, rank, entry):
        ...
        self.connection.execute('INSERT INTO entries VALUES(?,?,?,?)', (word,rank,zlib.compress(payload,9),len(payload)))
        self.words.add(word)
    ...
    def finish(self, aliases, provenance, base_url):
        ...
        return {'id':self.id,'version':self.version,'schemaVersion':SCHEMA,'entryCount':len(self.words),
                'byteCount':self.file.stat().st_size,'sha256':hashlib.file_digest(self.file.open('rb'),'sha256').hexdigest(),
                'url':base_url.rstrip('/')+'/'+self.file.name}
...
def build(version,base_url):
    ...
    words=sorted(set(english)|{w for w,r in rows.items() if r['definition'] or r['tag'] or r['oxford']=='1'})
    ...
    provenance={'ecdict':ECDICT_REV,'ipa':IPA_REV,'wordnet':NLTK_REV,
                'source_sha256':{name:hashlib.file_digest((CACHE/name).open('rb'),'sha256').hexdigest() for name in SOURCES}}
    ...
    packs.append(ce.finish(traditional_aliases,{'source_url':CEDICT_URL,'source_sha256':hashlib.file_digest(cedict.open('rb'),'sha256').hexdigest(),
    ...
    english_notice=('InstaDict dictionary sources\n\nEnglish definitions, examples and synonyms: WordNet 3.0, Princeton University.\n'
       ...
       'Modified by selecting records, parsing definitions, normalizing legacy IPA and compiling separate SQLite packs. '
    ...
    chinese_notice=('Chinese–English: CC-CEDICT, community contributors, published by MDBG.\n'
       ...
       'and compiled a compressed SQLite database. This derived dictionary pack is licensed under CC BY-SA 4.0. '
       'The unchanged source snapshot is provided alongside it as cc-cedict-source.txt.gz.\n\n'
    ...
    for identifier,notice in [('english-english',english_notice),('english-chinese',english_notice),('chinese-english',chinese_notice)]:
        ...
        pack['sha256']=hashlib.file_digest(database.open('rb'),'sha256').hexdigest()
    for target in (OUTPUT/'manifest.json',SHARED/'DictionaryCatalog.json'):
        target.write_text(json.dumps(catalog,ensure_ascii=False,indent=2)+'\n')
    shutil.copyfile(cedict,OUTPUT/'cc-cedict-source.txt.gz')
...
if __name__=='__main__':
    ...
    parser.add_argument('--base-url',default='https://fastcdn.candyrect.com/instadict')
