// Benchmark-only reader derived from Shared/DictionaryStore.swift.
// Keeps connection lifecycle, aliases, suggestions and presentation identical.
import Foundation
import SQLite3
import zlib
...
struct BenchmarkStore: Sendable {
    ...
    init(databaseURL: URL?, format: Format) {
    ...
    private func readEntry(_ word: String, database: OpaquePointer) throws -> DictionaryEntry? {
        let sql = format == .blocks128
            ? "SELECT b.payload,b.payload_size,e.byte_offset,e.payload_size FROM entries e JOIN blocks b ON b.id=e.block_id WHERE e.word=?"
        ...
        return try withStatement(sql, [word], database: database) { statement in
            ...
            let offset = Int(sqlite3_column_int(statement, 2))
            let entrySize = Int(sqlite3_column_int(statement, 3))
            guard (1...2_000_000).contains(size), offset >= 0, entrySize > 0,
                  offset <= size, entrySize <= size - offset,
                  let source = sqlite3_column_blob(statement, 0) else { throw StoreError.invalidEntry }
            let storedSize = Int(sqlite3_column_bytes(statement, 0))
            var data: Data
            if format == .uncompressed {
                guard storedSize == size else { throw StoreError.invalidEntry }
                data = Data(bytes: source, count: size)
            } else {
                ...
                let status = data.withUnsafeMutableBytes { destination in
                    uncompress(destination.bindMemory(to: Bytef.self).baseAddress, &outputSize,
                               source.assumingMemoryBound(to: Bytef.self), uLong(storedSize))
                }
                guard status == Z_OK, outputSize == size else { throw StoreError.invalidEntry }
            }
            // Byte offsets select the one entry to decode; never decode all 128.
            let entryData = format == .blocks128 ? data.subdata(in: offset..<(offset + entrySize)) : data
            guard let entry = try? JSONDecoder().decode(DictionaryEntry.self, from: entryData) else {
